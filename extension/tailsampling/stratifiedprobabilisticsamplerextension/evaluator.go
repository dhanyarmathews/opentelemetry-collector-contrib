...
type StratifiedProbabilisticEvaluator struct {
    mu                    sync.Mutex
    logger                *zap.Logger
    threshold             uint64
...
/*func (s *StratifiedProbabilisticEvaluator) Evaluate(
    _ context.Context,
    traceID pcommon.TraceID,
    traceData *samplingpolicy.TraceData,
) (samplingpolicy.Decision, error) {

    s.logger.Debug("StratifiedProbabilisticEvaluator called",
        zap.String("traceID", traceID.HexString()),
    )

    // TODO: put your real sampling logic here

    return samplingpolicy.DecisionSampled, nil
}
*/
...
func (s *StratifiedProbabilisticEvaluator) Evaluate(_ context.Context, traceID pcommon.TraceID, traceData *samplingpolicy.TraceData) (samplingpolicy.Decision, error) {
        ...
        r := sampling.TraceIDToRandomness(traceID)
        rnd := newTraceIDW3CSpecMethod(r)
        //rnd := traceIDW3CSpecMethod(sampling.TraceIDToRandomness(traceID))
        //rnd := traceIDW3CSpecMethod{randomnessMethod(sampling.TraceIDToRandomness(traceID)),}

        s.logger.Debug("Randomness check:", zap.Uint64("Randomness", rnd.randomness().Unsigned()), zap.Uint64("Threshold", s.threshold))

        value := rnd.randomness().Unsigned()
...
func (s *StratifiedProbabilisticEvaluator) getTraceTrajectoryHash(traceData *samplingpolicy.TraceData) (string, error) {
...
func (s *StratifiedProbabilisticEvaluator) getTraceSpanDetails(traceData *samplingpolicy.TraceData) ([]Node, []Edge) {
