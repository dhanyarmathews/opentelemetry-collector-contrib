...
/*func (p *spanIntentProcessor) processTraces(ctx context.Context, td ptrace.Traces) (ptrace.Traces, error) {
	...
	startTime := time.Now()
	const decayFactor = 0.97 // tune this for decay speed
...
func (p *spanIntentProcessor) processTracesForSampling(
    ...
    // Deduplicate based on priority: failed > degraded > normal
    ...
    p.logger.Info("[NOTRACE_FNV] Traces classified", zap.Int("normal", len(normalSet)), zap.Int("degraded", len(degradedSet)), zap.Int("failed", len(failedSet)),)
    ...
    // Step 1: Fully allocate bias==1 categories
    ...
    // Step 2: Proportional allocation for others
    for label, traces := range traceGroups {
        if biasMap[label] < 1 {
    ...
    // Step 3: Apply reservoir sampling per category
    for label, traces := range traceGroups {
        ...
        // Build scored trace list
        type scoredTrace struct {
            tid   pcommon.TraceID
            score uint64
        }

        reservoir := make([]scoredTrace, 0, budget)

        for i, tid := range traces {
            score := hashTraceID(tid)

            st := scoredTrace{tid: tid, score: score}

            if len(reservoir) < budget {
                reservoir = append(reservoir, st)
            } else {
        ...
        // Convert reservoir to map for fast lookup
        selected := make(map[pcommon.TraceID]struct{}, len(reservoir))
        for _, st := range reservoir {
            selected[st.tid] = struct{}{}
        }

        // Emit metrics and forward/drop traces
...
func (p *spanIntentProcessor) processTraces(ctx context.Context, td ptrace.Traces) (ptrace.Traces, error) {
    ...
    for i := 0; i < resourceSpans.Len(); i++ {
		...
		//}
                ...
                //modelTD, exists := p.modelTDigestMap[key]
                //if !exists {
                //    modelTD = &decayingTDigest{
                //        td:          utility.NewTDigest(100),
                //        decayFactor: 0.97,
                //        weightSum:   0,
                //    }
                //    p.modelTDigestMap[key] = modelTD
                //}

                //q75 := modelTD.Quantile(0.75)
                //q95 := modelTD.Quantile(0.95)
                p.tdigestMutex.Unlock()
